...
class _ReadPageState extends State<ReadPage> {
  ...
  void switchChapter1(int seqNo) {
    ...
    DatabaseHelper.db.getChapterByBookId(book.id).then((value) async {
      ...
      var start = currentChapterIndex - 50;
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        _pageController.jumpToPage(
            chapterTitlePageNumMap[_nowChapter.value]! + nowChapterPage);
        _currentPage.value =
            chapterTitlePageNumMap[_nowChapter.value]! + nowChapterPage;
        isLoading = false;
      });
  ...
  void switchChapter(int seqNo, bool isAfter) {
    isLoading = true;
    ...
    DatabaseHelper.db.getChapterByBookId(book.id).then((value) async {
      ...
      for (var i = currentChapterIndex; i <= currentChapterIndex; i++) {
        var chapter = chapterList[i];
        var chapterContent =
        await BookUtils.loadBook(join(dataDir.path, chapter.path));
        ...
        var afterAddLength = pageList.length;

        if (seqNo == chapter.seqNo) {
          page = afterAddLength - beforeAddLength;
        }
      ...
      setState(() {
        ...
        if (isAfter) {
          ...
          widgetList.insertAll(0, pageList);
        }
      });
      ...
      if (hasChapterTitle) {
        widgetList.add(Text(
          ...
          style: TextStyle(
              ...
              fontWeight: fontWeightList[titleFontWeight]),
        ));
      } else {
        if (text.startsWith('       ')) {
          ...
          if (i + 1 < textList.length && textList[i + 1]['text'].trim().length != 0) {
            List<Widget> tt = [];
            tt.add(Row(
              children: [
                SizedBox(
            ...
            for (var j = 0; j < textList1.length; j++) {
              ...
              rowChildren.add(Text(
                ...
                style: TextStyle(
                    ...
                    fontFamily: fontFamily,
                    color: Color(fontColor),
                    fontWeight: fontWeightList[contentFontWeight]
                ),
              ));
            }
            tt.add(Expanded(
                            ...
                            child: ReadFontSetting(
                              ...
                              updateFunc: (FontSetting fontSetting) {
                                setState(() {
                                  ...
                                  titleFontWeight = fontSetting.titleFontWeight;
                                  contentFontWeight = fontSetting.contentFontWeight;
                                  switchChapter1(currentSeqNo.value);
                                });
                              },
                            ))
