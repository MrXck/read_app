...
class _ReadOutSidePageState extends State<ReadOutSidePage> {
  ...
  void switchChapter1(int seqNo) async {
    ...
    List<List<String>?> textPageList = List.generate(
      maxContentPage,
      (index) => null,
    );
    ...
    for (var i = start; i <= end; i++) {
      ...
      Map pageMap = calcPage(chapterContent, height, width, settings.fontSize);
    ...
    setState(() {
      if (isAfter) {
        ...
        for (int i = pageList.length - 1; i >= 0; i--) {
          ...
          pageTextList[startHasContentPage - (pageList.length - i)] =
              textPage[i];
        }
        startHasContentPage -= pageList.length;
        if (startHasContentPage - 1 >= 0) {
      ...
      if (hasChapterTitle) {
        widgetList.add(
          ValueListenableBuilder(
            valueListenable: nowSpeakLine,
            builder: (BuildContext context, String value, Widget? child) {
              var isHighLight = false;
              if (value.isNotEmpty &&
                  text.trim().replaceFirst(zhanwei, '') == value) {
                isHighLight = true;
              }
              return Container(
            ...
            widgetList.add(
              ValueListenableBuilder(
                ...
                builder: (BuildContext context, String value, Widget? child) {
                  var isHighLight = false;
                  if (value.isNotEmpty &&
                      text.replaceFirst(zhanwei1, '') == value) {
                    isHighLight = true;
                  }
                  return Container(
                    color: isHighLight ? Colors.yellow : Colors.transparent,
  ...
  Map calcPage(String data, double height, double width, double fontSize) {
    ...
    return {'pageList': pageList, 'textPage': textPage};
  }
